...
export default function BuyerSettings() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (mandateRes.data.mandates?.length > 0) {
          setMandate({ dailyBudgetLimit: mandateRes.data.mandates[0].dailyBudgetLimit });
        }
        if (shippingRes.data.shipping) {
          setShipping(shippingRes.data.shipping);
        }
      } catch (err) {
        console.error(err);
      }
  ...
  const handleSave = async () => {
    ...
    try {
      await Promise.all([
        apiClient.put('/api/dashboard/mandate', { dailyBudgetLimit: mandate.dailyBudgetLimit }),
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <Card className="p-8 bg-card border-border/50 shadow-sm">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              <h2 className="text-xl font-semibold text-foreground">Auto-Cart Mandate</h2>
          ...
          <div className="space-y-4 max-w-xl">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground pt-1">
                Purchases exceeding this limit will be placed in your Approval Inbox for manual 1-click confirmation.
              </p>
            </div>
          </div>
        </Card>
